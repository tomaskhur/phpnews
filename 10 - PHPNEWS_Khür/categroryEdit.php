...
<?php
    if (isset($_POST) && !empty(($_POST)))
    {
        if (isset($categoryId))
        {
            $cr->updateCategory($categoryId, $_POST['name']);
...
<div class="admin-form-editor">
    <form method="post">
        <?php
            if (isset($categoryId))
            {
                $category = $cr->getCategory($categoryId);
            }
        ?>
        <label>
            Jméno kategorie:</label><input name="name" type="text" value="<?= isset($categoryId) ? $category['name'] : '' ?>">
        <button class="mb-5" type="submit">Uložit</button>
    </form>
</div>
