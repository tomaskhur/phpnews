...
<head>
    ...
    <title>PHP NEWS - Články autora</title>
</head>
<body>

<?php
    $authorId = $_GET['id'];
    $authorArticles = $authorRepository->getAuthorArticles($authorId);
    $authorName = $authorRepository->getAuthor($authorId);
?>
...
<nav class="my-navbar navbar p-3 navbar-expand-lg">
    ...
    <div class="header-container">
        <h1 class="fw-semibold">Články od redaktora</h1>
        <h5 class="fw-semibold"> <?= $authorName['name'] .' '. $authorName['surname']  ?> </h5>
    </div>

    <?php if (empty($authorArticles)): ?>
        <div class="container">
            <div class="row">
                    <h3 class="fw-semibold mt-5 d-flex justify-content-center ">Tento redaktor zatím žádné články nepřidal.</h3>
            </div>
        </div>
    <?php endif; ?>

    <?php foreach ($authorArticles as $a): ?>
        <?php $formatDate = date("d.m.Y H:i", strtotime($a['created_at'])) ?>
    <main>
        <div class="mt-5 articles">
            <?php foreach (explode(',', $a['categoryName']) as $key => $categoryName): ?>
                <a href="categoriesList.php?id=<?= explode(',',$a['categoryId'])[$key] ?>"><?= $categoryName ?></a>
            <?php endforeach;?>
            <h3 class="fw-semibold"><a href="articleDetail.php?id=<?= $a['id'] ?>"><?= $a['title'] ?></a></h3>
            <h5><?= $a['perex'] ?></h5>
            <a class="article-read-more" href="articleDetail.php?id= <?= $a['id'] ?>">Číst dále <i class="fa-solid fa-arrow-right"></i></a>
            <p><?= $formatDate ?></p>
        </div>
        <?php endforeach;?>
    </main>
