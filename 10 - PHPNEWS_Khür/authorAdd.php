...
<header>
    <div class="admin-header-container">
            ...
            <a href="authorEdit.php" class="btn btn-primary">Přidat redaktora</a>
    </div>
</header>

<main>

    <div class="admin-all-editors">
        <?php $authors = $authorRepository->getAuthors() ?>
        <h2 class="mt-5 fw-semibold">Skupina redaktorů</h2>
        <table>
            <thead>
            <tr>
                <th>Jméno</th>
            ...
            <?php $count = $authorRepository->cannotDeleteAuthor($author['id']) ?>
                <tr>
                    <td>
                        <?php if($author['visible'] == 0): ?>
                            <del><?= $author['name'] ?></del>
                        <?php elseif ($author['visible'] == 1): ?>
                            <?= $author['name'] ?>
                        <?php endif; ?>
                    </td>
                    <td>
                        <?php if($author['visible'] == 0): ?>
                            <del><?= $author['surname'] ?></del>
                        <?php elseif ($author['visible'] == 1): ?>
                            <?= $author['surname'] ?>
                        <?php endif; ?>
                    </td>
                    <td><a href="authorEdit.php?id=<?= $author['id'] ?>">Upravit</a></td>
                    <?php if ($count['authorCount'] > 0): ?>
                        <td><a style="color: gray" href="" onclick="return alert('Autor je přiřazen článku/ům. Nejdříve odstraňte všechny články obsahující autora')">Smazat</a></td>
                    <?php else: ?>
                        <td><a href="authorDelete.php?id=<?= $author['id'] ?>">Smazat</a></td>
                    <?php endif; ?>
                </tr>
            <?php endforeach; ?>
        </table>
