...
<head>
    ...
    <title>PHP NEWS - Detail článku</title>
</head>
<body>
...
<!--            GENERATE ARTICLE FOR CATEGORY CONNECTION    -->


        <h1 class="fw-semibold"><?= $article['title'] ?></h1>
        ...
        <p><?= $formatDate ?> <a href="authorDetail.php?id=<?= $article['authorId'] ?>"><?= $article['authorName'] ?> <?= $article['authorSurname'] ?></a></p>
    </div>

        <div class="articles">
            <?php
                if (!empty($article['file_name'])): ?>
                    <img class="mb-3" src="<?= $article['file_name']?>" alt="article image" style="height: 300px; width: 300px">
                <?php endif; ?>
            <p><?= $article['text'] ?></p>
            <a href="mpdf.php?id=<?=$articleId?>">Vytisknout do PDF <i class="fa-solid fa-download"></i></a>
        </div>
</main>
